...
public class TaskManagementController {
    ...
    @PutMapping("/update-priorities")
    public Response<List<TaskManagementDto>> updateTaskPriorities(@RequestBody UpdatePriorityRequest request) {
        return new Response<>(taskManagementService.updateTasksPriority(request));
    }
    ...
    public Response<List<TaskManagementDto>> getTasksByPriority(@PathVariable Priority priority) {
        return new Response<>(taskManagementService.getTasksViaPriority(priority));

    }

    // ✅ [NEW] Get full task detail (activity + comments)
    @GetMapping("task/{id}/detail")
    public Response<TaskDetailDto> getTaskDetail(@PathVariable Long id) {
        return new Response<>(taskManagementService.getTaskDetails(id));
    }

    // ✅ [NEW] Add comment to a task
    @PostMapping("task/{id}/comments")
    public Response<Void> addComment(@PathVariable Long id, @RequestBody TaskComment request) {
        taskManagementService.addComment(id, request.getUserId(), request.getMessage());
